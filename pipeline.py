...
from utils.metrics import aggreate, batch_metric_calculation, ambiguity_differentiation
...
class CWNPipe():
    ...
    def answer_with_cp(self,
                       tokens_logits):  # take in CP set only options whose logits are greater then CP value. CP value is defined through running calibration.py script
        possible_options = []
    ...
    def answer_prompt(self, prompts_di, description, task, prefix, action):
    ...
    def generate_answer_batch(self, prompts, batch_size=2):  # choosing CP set for batch
        llm = LLM(self.config.answering['model'], self.config.answering['generation_kwargs'])
        full_texts = []
        full_logits = []
        for i in tqdm.tqdm(range(0, len(prompts), batch_size)):
        ...
        for i in range(len(full_texts)):
            filtered_logits = llm.filter_logits(full_logits[i][0],
                                                words=["A", "B", "C", "D", 'a', 'b', 'c', 'd', '1', '2', '3', '4'])
    ...
    def generate_answer(self, prompt, description, task, prefix, action):  # choosing CP set for single example
        ...
        filtered_logits = llm.filter_logits(logits[-1][0],
                                            words=["A", "B", "C", "D", 'a', 'b', 'c', 'd', '1', '2', '3', '4'])
        llm = None
    ...
    def run(self, description, task, prefix, action):
        ...
        answers_letter, prompt = self.generate_answer(options, description, task, prefix, action)[1]
    ...
    def run_batch(self, option_prompts, tasks_for_ans):  # run, but for batch
        ...
        for i in range(len(options)):
            answer_prompts.append(
                self.answer_prompt(options[i], tasks_for_ans[i]['description'], tasks_for_ans[i]['task'],
                                   tasks_for_ans[i]['prefix'], tasks_for_ans[i]['action']))
...
if __name__ == "__main__":
    ...
    print()
    cwn_config = CWNConfig(configs)
    cwn = CWNPipe(cwn_config)
    dataset = pd.read_csv("data/AmbiK_data.csv")  # ambik_test_400.csv #ambik_test_for_testing.csv
    amb = dataset[
        ['Unnamed: 0', 'environment_short', 'environment_full', 'ambiguity_type', 'amb_shortlist', 'ambiguous_task', 'question',
         'answer', 'plan_for_amb_task', 'start_of_ambiguity', 'user_intent']]
    # , (environment_short, environment_full, unambiguous_direct, unambiguous_indirect, ambiguity_type, amb_shortlist,
    #    ambiguous_task, question, answer, plan_for_clear_task, plan_for_amb_task, start_of_ambiguity, user_intent, variants)

    # "id,environment_full,task,intent_object,intent_location,environment_short,ambiguity_type,user_intent"
    ...
    dataset = dataset.drop(
        columns=['Unnamed: 0', 'unambiguous_direct', 'unambiguous_indirect', 'ambiguous_task', 'plan_for_clear_task',
                 'plan_for_amb_task', 'variants'])
    ...
    for i in range(len(dataset)):  # len(dataset)
        ...
        plan = str(dataset.loc[i, 'plan']).replace('\r', '').split('\n')
        point = dataset.loc[i, 'start_of_ambiguity']
        # print(point, plan)
        if pd.isna(point) or point == 0:
            prefix = 'Your first action is:'
            point = 0
        else:
            prefix = 'Your previous actions were:\n'
            for act in plan[:int(point)]:
                prefix += act
                prefix += '\n'
        action = plan[int(point)]

        tasks_for_ans.append({'description': description, 'task': task, 'prefix': prefix, 'action': action})
        option_prompt = cwn.options_prompt(description, task, prefix, action)
        option_prompts.append(option_prompt)

    options, right_answers = cwn.run_batch(option_prompts, tasks_for_ans)
    print(right_answers)
    batch_size = 2
    metrics_batch = batch_metric_calculation(llm_answers_batch=right_answers, scores=options, y_amb_type_batch=amb_type,
                                             y_amb_intents_batch=intents, y_amb_shortlist_batch=amb_shortlist)
    ...
    agg_metrics_df.to_csv(f"data/cwn_agg_metrics_{i}.csv")  # поправить записььь
    ...
    print(amb_dif)
    with open(f"data/cwn_ambdif_{i}.txt", 'a') as file:
        file.write(str(amb_dif))

    metrics.to_csv(f"data/cwn_metrics_{i}.csv")