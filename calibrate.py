...
from utils.parse_config import parse_args, parse_config
from pipeline import CWNConfig, CWNPipe


def get_logits(cwn, description, task, prefix, action):  # def get_logits(knowno, prompt)
    options = cwn.predict_examples(description, task, prefix, action)
    gc.collect()
    choose = cwn.generate_answer(options, description, task, prefix, action)
    gc.collect()
    # print('options')
    # print(options)
    print(choose[0])
